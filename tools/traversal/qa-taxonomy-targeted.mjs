...
const out='tools/traversal/qa/taxonomy-v1/targeted';await mkdir(out,{recursive:true});
const browser=await chromium.launch({headless:true});
const results=[];
for(const seed of [0,1,2]){
 const page=await browser.newPage({viewport:{width:960,height:720}});
 await page.goto(`http://127.0.0.1:5177/tools/traversal/taxonomy-review.html?seed=${seed}`);
 await page.waitForFunction(()=>window.review);
 await page.evaluate(()=>{const {scene,settle}=window.review;scene.moveToLane(1);scene.advance(20);settle();scene.element.querySelector('[data-traversal-confirm]').click();settle();scene.advance(12);settle();});
