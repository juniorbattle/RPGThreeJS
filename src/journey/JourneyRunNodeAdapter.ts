...
export function planJourneyBoundary(
  ...
  if (only) {
    ...
    return {
      ...
      presentation: {
        ...
        title: only.label,
        caption: `${meta.label} · ${meta.difficulty}`,
        ...(context.currentLabel ? { context: `Depuis : ${context.currentLabel}` } : {}),
        choices: [],
  ...
  if (!available.length) {
    return {
      ...
      presentation: {
        mode: 'terminal',
        eyebrow: 'Fin de route',
        title: 'La chronique s’achève ici',
