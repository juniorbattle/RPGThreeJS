...
import { installVfxComposerPanel } from './vfx/CombatVfxComposerPanel';
...
const notifyCampaignResult=victory=>{
  for(const unit of G.units)disposeStatusIndicators(unit);
...
const unitFocus=new UnitFocusController({
  dimOpacity:COMBAT_PRESENTATION.units.focusDimOpacity,
  bossDimFloor:COMBAT_PRESENTATION.units.focusBossFloor,
  eliteDimFloor:COMBAT_PRESENTATION.units.focusEliteFloor,
});
function restoreUnitFocus(){
  unitFocus.restore();
  for(const u of G.units){ if(u.alive)continue; const state=getUnitVisualState(u.team,u.alive,u.downed); if(u.grp)u.grp.visible=state.visible; if(u.mat)u.mat.opacity=state.bodyOpacity; if(u.blob&&u.blob.material)u.blob.material.opacity=state.shadowOpacity; }
}
...
const EXTERNAL_SPRITE_HEIGHTS={
  '/assets/characters/pixel/masters/serpent_raider.png':1.76,
  '/assets/characters/pixel/masters/serpent_brute.png':1.76,
  '/assets/characters/pixel/masters/serpent_oracle.png':1.76,
  '/assets/characters/pixel/masters/serpent_duelist_elite.png':2.08,
  '/assets/characters/pixel/masters/serpent_general_boss.png':2.42,
  '/assets/characters/pixel/masters/wolf.png':1.42,
  '/assets/characters/pixel/masters/venom_serpent.png':1.48,
  '/assets/characters/pixel/masters/forest_spider.png':1.34,
  '/assets/characters/pixel/masters/forest_badger.png':1.32,
  '/assets/characters/pixel/masters/marsh_toad.png':1.36,
  '/assets/characters/pixel/masters/cave_rat.png':1.24,
  '/assets/characters/pixel/masters/wild_boar.png':1.42,
  '/assets/characters/pixel/masters/goblin.png':1.72,
  '/assets/characters/pixel/masters/skeleton.png':1.9,
  '/assets/characters/pixel/archive/non-demo/troll.png':2.3,
...
function buildGridOverlay(){
  ...
  for(let i=0;i<=CFG.D;i++){ const z=(i-CFG.D/2)*CFG.TILE; addBand(glowX,glowMat,0,z,COMBAT_RENDER_LAYERS.TILE_BASE); addBand(underX,underMat,0,z,COMBAT_RENDER_LAYERS.TILE_BASE+.1); addBand(coreX,coreMat,0,z,COMBAT_RENDER_LAYERS.TILE_BASE+.2); }
  for(let i=0;i<=CFG.W;i++){ const x=(i-CFG.W/2)*CFG.TILE; addBand(glowZ,glowMat,x,0,COMBAT_RENDER_LAYERS.TILE_BASE); addBand(underZ,underMat,x,0,COMBAT_RENDER_LAYERS.TILE_BASE+.1); addBand(coreZ,coreMat,x,0,COMBAT_RENDER_LAYERS.TILE_BASE+.2); }
  ...
  for(let ix=0;ix<=CFG.W;ix++)for(let iz=0;iz<=CFG.D;iz++){ const k=new THREE.Mesh(knotGeo,knotMat); k.rotation.x=-Math.PI/2; k.rotation.z=Math.PI/4; k.position.set((ix-CFG.W/2)*CFG.TILE,y+.012,(iz-CFG.D/2)*CFG.TILE); k.renderOrder=COMBAT_RENDER_LAYERS.TILE_BASE+.3; worldRoot.add(k); }
  ...
  const lines=new THREE.LineSegments(geo,material);lines.renderOrder=COMBAT_RENDER_LAYERS.TILE_BASE+.4;worldRoot.add(lines);G.gridLines=lines;
}
async function buildWorld(){
  G.backgroundLayers=new BackgroundLayerSystem(scene);
  await G.backgroundLayers.load(OPTION_C_PROOF_ACTIVE?OPTION_C_STRATEGIC_BACKGROUND:combatBackgroundFor(COMBAT_SCENE_ID));
  ...
  if(COMBAT_PRESENTATION.arena.showBaseGridBorders||(GRID_DEBUG_ENABLED&&COMBAT_PRESENTATION.arena.showDebugGridBorders))buildGridOverlay();
}
...
function addRingHL(gx,gz,color,op=0.62,ringScale=1,cgx=null,cgz=null){ const c=cellAt(gx,gz); if(!c)return;
  ...
  u.rotation.x=-Math.PI/2; u.position.set(px,c.topY+0.094,pz); u.renderOrder=COMBAT_RENDER_LAYERS.TILE_TARGET+1.5; u.scale.setScalar(ringScale); u.userData.baseOp=Math.min(.76,op*.76); u.userData.pulse=0.026; hlGroup.add(u); hlMeshes.push(u);
  ...
  m.rotation.x=-Math.PI/2; m.position.set(px,c.topY+0.100,pz); m.renderOrder=COMBAT_RENDER_LAYERS.TILE_TARGET+2; m.scale.setScalar(ringScale); m.userData.baseOp=Math.min(1,op*1.12); m.userData.pulse=0.04; hlGroup.add(m); hlMeshes.push(m); return m; }
function cellKey(gx,gz){ return gx+','+gz; }
function addInvalidTiles(valid,skipActive=false){ if(!GRID_DEBUG_ENABLED)return; for(let gx=0;gx<CFG.W;gx++)for(let gz=0;gz<CFG.D;gz++){ if(valid.has(cellKey(gx,gz)))continue; if(skipActive&&G.active&&G.active.gx===gx&&G.active.gz===gz)continue; addHL(gx,gz,0x7d8066,COMBAT_PRESENTATION.arena.invalidTileOpacity,'invalid'); } }

// cursor ring
let cursorMesh=null,cursorUnderMesh=null;
function buildCursor(){ const ug=new THREE.RingGeometry(0.34,0.54,72),g=new THREE.RingGeometry(0.39,0.5,72); cursorUnderMesh=new THREE.Mesh(ug,new THREE.MeshBasicMaterial({color:0x080604,transparent:true,opacity:.62,side:THREE.DoubleSide,depthWrite:false,fog:false,toneMapped:false})); cursorUnderMesh.rotation.x=-Math.PI/2; cursorUnderMesh.renderOrder=COMBAT_RENDER_LAYERS.TILE_TARGET+2.5; cursorUnderMesh.visible=false; scene.add(cursorUnderMesh); cursorMesh=new THREE.Mesh(g,new THREE.MeshBasicMaterial({color:0xfff0c8,transparent:true,opacity:.96,side:THREE.DoubleSide,depthWrite:false,fog:false,toneMapped:false})); cursorMesh.rotation.x=-Math.PI/2; cursorMesh.renderOrder=COMBAT_RENDER_LAYERS.TILE_TARGET+3; cursorMesh.visible=false; scene.add(cursorMesh); }
...
// ============================= UNIT DEFINITIONS =============================
const DEFS=[
  {team:'player',kind:'knight', name:'Chevalier',className:'Guerrier',portrait:'/assets/characters/pixel/masters/alistair.png',hp:140,str:20,mag:3, end:18,dex:9, cha:10,mov:2, weapons:[{name:'Épée',icon:'⚔️',type:'phys',min:1,max:1,power:10,crit:0.10,acc:0.95}], skills:['w_break_guard','w_charge','w_whirl','w_lion_surge'], gx:0,gz:0},
  {team:'player',kind:'cleric', name:'Clerc',className:'Mage Blanc',portrait:'/assets/characters/pixel/masters/white_mage.png',    hp:100,str:6, mag:22,end:10,dex:11,cha:18,mov:2, weapons:[{name:'Masse',icon:'🔨',type:'phys',weaponType:'crosier',min:1,max:1,power:8,crit:0.06,acc:0.92}], skills:['w_salvation','w_purify','w_sanctuary','w_miracle'], gx:0,gz:1},
  {team:'player',kind:'mage',   name:'Mage',className:'Mage Noir',portrait:'/assets/characters/pixel/masters/dark_mage.png',     hp:75, str:5, mag:28,end:7, dex:12,cha:14,mov:2, weapons:[{name:'Bâton',icon:'🪄',type:'phys',min:1,max:1,power:8,crit:0.06,acc:0.95}], skills:['n_dark_bolt','n_teleport','n_flame_wave','n_dark_meteor'], gx:1,gz:2},
  {team:'player',kind:'archer', name:'Archère',className:'Archer',portrait:'/assets/characters/pixel/masters/archer.png',  hp:90, str:14,mag:3, end:9, dex:22,cha:10,mov:3, weapons:[{name:'Arc',icon:'🏹',type:'phys',min:2,max:4,power:9,crit:0.10,acc:0.92}], skills:['a_precise_shot','a_hawk_leap','a_arrow_rain','a_zenith_arrow'], gx:1,gz:3},
  {team:'player',kind:'knight', name:'Paladin',className:'Paladin',portrait:'/assets/characters/pixel/archive/non-demo/aldric.png',hp:140,str:16,mag:14,end:17,dex:9, cha:14,mov:2,weapons:[{name:'Lame sainte',icon:'⚔',type:'phys',min:1,max:1,power:10,crit:0.06,acc:0.93}],skills:['p_holy_strike','p_interpose','p_oathwall','p_radiant_judgement'],gx:0,gz:2},
  {team:'player',kind:'darkmage',name:'Morvan',className:'Chevalier Noir',portrait:'/assets/characters/pixel/archive/non-demo/morvan.png',hp:130,str:18,mag:15,end:14,dex:11,cha:8,mov:2,weapons:[{name:'Lame maudite',icon:'⚔',type:'phys',min:1,max:1,power:11,crit:0.08,acc:0.92}],skills:['d_cursed_blade','d_void_step','d_blood_pact','d_devouring_eclipse'],gx:0,gz:3},
  {team:'player',kind:'mage',   name:'Lyra',className:'Mage Rouge',portrait:'/assets/characters/pixel/archive/non-demo/lyra.png',hp:90,str:12,mag:21,end:9, dex:13,cha:14,mov:2,weapons:[{name:'Bâton rouge',icon:'✦',type:'mag',min:1,max:2,power:9,crit:0.06,acc:0.95}],skills:['r_arcane_blade','r_rune_step','r_scarlet_circle','r_perfect_duality'],gx:1,gz:0},
  {team:'player',kind:'cleric', name:'Eldwin',className:'Enchanteur',portrait:'/assets/characters/pixel/archive/non-demo/eldwin.png',hp:85,str:5,mag:23,end:9, dex:12,cha:20,mov:2,weapons:[{name:'Sceau',icon:'✦',type:'phys',min:1,max:1,power:7,crit:0.04,acc:0.94}],skills:['e_vigor_rune','e_transpose','e_binding_seal','e_absolute_harmony'],gx:1,gz:1},
  {team:'player',kind:'rogue',  name:'Talon',className:'Ninja',portrait:'/assets/characters/pixel/archive/non-demo/talon.png',hp:100,str:15,mag:10,end:10,dex:26,cha:10,mov:3,weapons:[{name:'Dague',icon:'†',type:'phys',min:1,max:1,power:8,crit:0.20,acc:0.95}],skills:['ni_venom_blade','ni_shadow_step','ni_smoke_bomb','ni_silent_assassin'],gx:2,gz:0},
  {team:'player',kind:'archer', name:'Gunnar',className:'Artilleur',portrait:'/assets/characters/pixel/archive/non-demo/gunnar.png',hp:100,str:15,mag:4,end:11,dex:19,cha:8,mov:2,weapons:[{name:'Arbalète',icon:'⌁',type:'phys',min:2,max:4,power:10,crit:0.08,acc:0.93}],skills:['ar_calibrated_shot','ar_explosive_retreat','ar_incendiary_grenade','ar_artillery_barrage'],gx:2,gz:1},
  {team:'foe',kind:'brigand', name:'Brigand',className:'Brigand',portrait:'/assets/characters/pixel/masters/serpent_raider.png',  hp:85, str:13,mag:3, end:10,dex:16,cha:6, mov:2, weapons:[{name:'Dague',icon:'🗡️',type:'phys',min:1,max:1,power:9,crit:0.18,acc:0.95}], skills:[], ai:'aggressive', gx:6,gz:0},
  {team:'foe',kind:'brigand', name:'Brigand',className:'Brigand',portrait:'/assets/characters/pixel/masters/serpent_raider.png',  hp:85, str:13,mag:3, end:10,dex:16,cha:6, mov:2, weapons:[{name:'Dague',icon:'🗡️',type:'phys',min:1,max:1,power:9,crit:0.18,acc:0.95}], skills:[], ai:'aggressive', gx:6,gz:3},
  {team:'foe',kind:'brute',   name:'Brute',className:'Brute',portrait:'/assets/characters/pixel/masters/serpent_brute.png',    hp:130,str:19,mag:3, end:17,dex:6, cha:5, mov:2, weapons:[{name:'Massue',icon:'🏏',type:'phys',min:1,max:1,power:13,crit:0.05,acc:0.85}], skills:['enemy_heavy_strike'], ai:'guardian', gx:7,gz:1},
  {team:'foe',kind:'darkmage',name:'Mage Noir',className:'Mage Noir',portrait:'/assets/characters/pixel/masters/serpent_oracle.png',hp:75, str:4, mag:20,end:9, dex:12,cha:8, mov:2, weapons:[{name:'Bâton',icon:'🪄',type:'mag',min:1,max:3,power:8,crit:0.05,acc:0.95}], skills:['enemy_dark_bolt','enemy_hex'], ai:'cautious', gx:7,gz:2}
];

const BOSS_DEFS=[
  {team:'foe',kind:'brute',name:'Général Serpent',className:'Général Serpent',portrait:'/assets/characters/pixel/masters/serpent_general_boss.png',hp:450,str:30,mag:8,end:24,dex:9,cha:12,mov:0,weapons:[{name:'Lame Serpent',icon:'⚔️',type:'phys',min:1,max:2,power:18,crit:0.08,acc:0.9}],skills:['enemy_hex','boss_guard','boss_quake','boss_titan_slam'],ai:'aggressive',gx:5,gz:1,size:2,immobile:true,boss:true},
  {team:'foe',kind:'knight',name:'Champion du Lion',className:'Champion du Lion',portrait:'/assets/characters/pixel/masters/lion_champion.png',hp:520,str:28,mag:6,end:27,dex:10,cha:16,mov:0,weapons:[{name:'Lame du Lion',icon:'⚔️',type:'phys',min:1,max:2,power:16,crit:0.06,acc:0.92}],skills:['enemy_heavy_strike','boss_guard','boss_quake','boss_apocalypse'],ai:'guardian',gx:5,gz:1,size:2,immobile:true,boss:true}
];
...
const VISUAL_UNIT_TEMPLATES={
  serpent_raider:{team:'foe',kind:'brigand', name:'Pillard Serpent',className:'Pillard Serpent',portrait:'/assets/characters/pixel/masters/serpent_raider.png',hp:85,str:13,mag:3,end:10,dex:16,cha:6,mov:2,weapons:[{name:'Dague',icon:'†',type:'phys',min:1,max:1,power:9,crit:0.18,acc:0.95}],skills:['enemy_binding_shot'],ai:'aggressive'},
  serpent_brute:{team:'foe',kind:'brute',name:'Brute Serpent',className:'Brute Serpent',portrait:'/assets/characters/pixel/masters/serpent_brute.png',hp:130,str:19,mag:3,end:17,dex:6,cha:5,mov:2,weapons:[{name:'Massue',icon:'◆',type:'phys',min:1,max:1,power:13,crit:0.05,acc:0.85}],skills:['enemy_heavy_strike'],ai:'guardian'},
  serpent_oracle:{team:'foe',kind:'darkmage',name:'Oracle Serpent',className:'Oracle Serpent',portrait:'/assets/characters/pixel/masters/serpent_oracle.png',hp:75,str:4,mag:20,end:9,dex:12,cha:8,mov:2,weapons:[{name:'Bâton',icon:'✦',type:'mag',min:1,max:3,power:8,crit:0.05,acc:0.95}],skills:['enemy_dark_bolt','enemy_hex'],ai:'cautious'},
  village_militia_spearman:{team:'foe',kind:'brigand',name:'Milicien de Bois-Clair',className:'Milicien lancier',portrait:'/assets/characters/pixel/masters/village_militia_spearman.png',hp:105,str:15,mag:2,end:13,dex:11,cha:8,mov:2,weapons:[{name:'Lance de milice',icon:'↟',type:'phys',min:1,max:2,power:9,crit:0.06,acc:0.92}],skills:[],ai:'guardian'},
  village_militia_slinger:{team:'foe',kind:'brigand',name:'Frondeur de Bois-Clair',className:'Milicien frondeur',portrait:'/assets/characters/pixel/masters/village_militia_slinger.png',hp:82,str:12,mag:2,end:9,dex:18,cha:8,mov:2,weapons:[{name:'Fronde',icon:'⌁',type:'phys',min:2,max:3,power:8,crit:0.10,acc:0.91}],skills:[],ai:'cautious'},
  serpent_elite_raider:{team:'foe',kind:'brigand', name:'Duelliste Serpent',className:'Duelliste Serpent',portrait:'/assets/characters/pixel/masters/serpent_duelist_elite.png',hp:300,str:22,mag:5,end:14,dex:20,cha:8,mov:0,weapons:[{name:'Lames volantes',icon:'†',type:'phys',min:2,max:3,power:13,crit:0.22,acc:0.95}],skills:['enemy_binding_shot','boss_fortify','boss_pin','boss_flurry'],ai:'aggressive',size:2,immobile:true,elite:true},
  serpent_elite_brute:{team:'foe',kind:'brute',name:'Massacreur Serpent',className:'Massacreur Serpent',portrait:'/assets/characters/pixel/masters/serpent_elite_brute.png',hp:380,str:28,mag:3,end:23,dex:5,cha:7,mov:0,weapons:[{name:'Lance-masse',icon:'◆',type:'phys',min:2,max:3,power:17,crit:0.06,acc:0.84}],skills:['enemy_venom_strike','boss_guard','boss_quake','boss_slam'],ai:'guardian',size:2,immobile:true,elite:true},
  serpent_duelist_elite:{team:'foe',kind:'brigand',name:'Duelliste Serpent',className:'Duelliste Serpent',portrait:'/assets/characters/pixel/masters/serpent_duelist_elite.png',hp:320,str:23,mag:5,end:15,dex:22,cha:8,mov:0,weapons:[{name:'Lames de jet',icon:'†',type:'phys',min:2,max:4,power:14,crit:0.24,acc:0.96}],skills:['enemy_binding_shot','boss_fortify','boss_pin','boss_flurry'],ai:'aggressive',size:2,immobile:true,elite:true},
  wolf:{team:'foe',kind:'brigand',name:'Loup',className:'Loup',portrait:'/assets/characters/pixel/masters/wolf.png',hp:80,str:13,mag:2,end:9,dex:20,cha:4,mov:3,weapons:[{name:'Morsure',icon:'◆',type:'phys',min:1,max:1,power:10,crit:0.12,acc:0.94}],skills:[],ai:'aggressive'},
  venom_serpent:{team:'foe',kind:'brigand',name:'Serpent venimeux',className:'Serpent venimeux',portrait:'/assets/characters/pixel/masters/venom_serpent.png',hp:90,str:12,mag:6,end:10,dex:15,cha:4,mov:2,weapons:[{name:'Crochets',icon:'◆',type:'phys',min:1,max:1,power:9,crit:0.1,acc:0.93}],skills:['enemy_venom_strike','enemy_hex'],ai:'cautious'},
  forest_spider:{team:'foe',kind:'brigand',name:'Araignee forestiere',className:'Araignee forestiere',portrait:'/assets/characters/pixel/masters/forest_spider.png',hp:68,str:11,mag:4,end:8,dex:19,cha:3,mov:3,weapons:[{name:'Mandibules',icon:'◆',type:'phys',min:1,max:1,power:8,crit:0.12,acc:0.93}],skills:['enemy_binding_shot'],ai:'aggressive'},
  forest_badger:{team:'foe',kind:'brigand',name:'Blaireau',className:'Blaireau',portrait:'/assets/characters/pixel/masters/forest_badger.png',hp:88,str:14,mag:2,end:13,dex:12,cha:3,mov:2,weapons:[{name:'Griffes',icon:'◆',type:'phys',min:1,max:1,power:10,crit:0.1,acc:0.92}],skills:[],ai:'aggressive'},
  marsh_toad:{team:'foe',kind:'brute',name:'Crapaud toxique',className:'Crapaud toxique',portrait:'/assets/characters/pixel/masters/marsh_toad.png',hp:108,str:13,mag:8,end:16,dex:6,cha:3,mov:2,weapons:[{name:'Langue lourde',icon:'◆',type:'phys',min:1,max:2,power:8,crit:0.04,acc:0.88}],skills:['enemy_venom_strike'],ai:'guardian'},
  cave_rat:{team:'foe',kind:'brigand',name:'Rat geant',className:'Rat geant',portrait:'/assets/characters/pixel/masters/cave_rat.png',hp:65,str:10,mag:2,end:7,dex:22,cha:2,mov:3,weapons:[{name:'Morsure',icon:'◆',type:'phys',min:1,max:1,power:8,crit:0.16,acc:0.94}],skills:[],ai:'aggressive'},
  wild_boar:{team:'foe',kind:'brute',name:'Sanglier',className:'Sanglier',portrait:'/assets/characters/pixel/masters/wild_boar.png',hp:116,str:18,mag:2,end:16,dex:8,cha:3,mov:2,weapons:[{name:'Charge',icon:'◆',type:'phys',min:1,max:1,power:12,crit:0.08,acc:0.9}],skills:['enemy_heavy_strike'],ai:'guardian'},
  goblin:{team:'foe',kind:'brigand',name:'Gobelin',className:'Gobelin',portrait:'/assets/characters/pixel/masters/goblin.png',hp:70,str:11,mag:3,end:8,dex:18,cha:5,mov:3,weapons:[{name:'Lance rouillée',icon:'↟',type:'phys',min:1,max:2,power:8,crit:0.08,acc:0.9}],skills:[],ai:'aggressive'},
  skeleton:{team:'foe',kind:'knight',name:'Squelette',className:'Squelette',portrait:'/assets/characters/pixel/masters/skeleton.png',hp:105,str:16,mag:3,end:15,dex:8,cha:3,mov:2,weapons:[{name:'Épée rouillée',icon:'⚔',type:'phys',min:1,max:1,power:10,crit:0.05,acc:0.88}],skills:['enemy_heavy_strike'],ai:'guardian'},
  troll:{team:'foe',kind:'brute',name:'Troll',className:'Troll',portrait:'/assets/characters/pixel/archive/non-demo/troll.png',hp:190,str:25,mag:2,end:23,dex:4,cha:4,mov:2,weapons:[{name:'Tronc',icon:'◆',type:'phys',min:1,max:1,power:16,crit:0.04,acc:0.82}],skills:['enemy_heavy_strike','enemy_crush'],ai:'guardian'},
  young_wyrm:{team:'foe',kind:'brute',name:'Jeune Wyrm',className:'Jeune Wyrm',portrait:'/assets/characters/pixel/masters/young_dragon_elite.png',hp:160,str:22,mag:16,end:18,dex:10,cha:8,mov:2,weapons:[{name:'Souffle court',icon:'✦',type:'mag',min:1,max:2,power:14,crit:0.06,acc:0.88}],skills:['enemy_dark_bolt','enemy_dragon_breath'],ai:'cautious'},
  forest_troll_elite:{team:'foe',kind:'brute',name:'Troll forestier',className:'Troll forestier',portrait:'/assets/characters/pixel/masters/forest_troll_elite.png',hp:420,str:30,mag:2,end:25,dex:5,cha:5,mov:0,weapons:[{name:'Jet de pierre',icon:'◆',type:'phys',min:2,max:4,power:18,crit:0.04,acc:0.84}],skills:['enemy_hex','boss_guard','boss_quake','boss_slam'],ai:'guardian',size:2,immobile:true,elite:true},
  young_dragon_elite:{team:'foe',kind:'brute',name:'Jeune dragon',className:'Jeune dragon',portrait:'/assets/characters/pixel/masters/young_dragon_elite.png',hp:400,str:24,mag:20,end:20,dex:12,cha:10,mov:0,weapons:[{name:'Souffle emeraude',icon:'✦',type:'mag',min:2,max:4,power:17,crit:0.07,acc:0.9}],skills:['enemy_hex','boss_regen','boss_freeze','boss_inferno'],ai:'cautious',size:2,immobile:true,elite:true},
  undead_champion:{team:'foe',kind:'knight',name:'Champion mort-vivant',className:'Champion mort-vivant',portrait:'/assets/characters/pixel/archive/non-demo/undead_champion.png',hp:350,str:26,mag:8,end:23,dex:8,cha:6,mov:0,weapons:[{name:'Onde froide',icon:'✦',type:'mag',min:2,max:3,power:15,crit:0.08,acc:0.9}],skills:['enemy_hex','boss_guard','boss_quake','boss_execution'],ai:'guardian',size:2,immobile:true,elite:true},
  ...
  seal_guardian:{team:'foe',kind:'darkmage',name:'Gardien du Sceau',className:'Gardien du Sceau',portrait:'/assets/characters/pixel/archive/non-demo/seal_guardian.png',hp:90,str:6,mag:20,end:11,dex:10,cha:16,mov:2,weapons:[{name:'Sceau',icon:'✦',type:'mag',min:1,max:3,power:8,crit:0.04,acc:0.94}],skills:['enemy_hex','boss_guard'],ai:'cautious'}
};
...
const INNATE_GIFTS_BY_WEAPON={
  ...
  long_spear:{name:'Percée',desc:"25% de chance de transpercer et frapper un second ennemi aligné."},
  grimoire:{name:'Flux de Mana',desc:"20% de chance de récupérer 2 AP après une attaque de base."},
  crosier:{name:'Main Bienveillante',desc:"Soigne les alliés au lieu de les blesser, inflige des dégâts aux ennemis."},
  rapier:{name:'Lame Hybride',desc:"Attaque physique de près, attaque magique à distance."},
  wand:{name:'Maléfice Aléatoire',desc:"25% de chance d'infliger une altération aléatoire à la cible."},
  longbow:{name:'Tir de Précision',desc:"+5% de précision à portée maximale."},
  shuriken:{name:'Double Frappe',desc:"25% de chance de lancer une seconde attaque gratuite."},
  dagger:{name:'Frappe Sournoise',desc:"Dégâts accrus de 45% en attaquant par derrière."},
...
const STATUS_BADGE_RENDER_ORDER=COMBAT_RENDER_LAYERS.UNIT_STATUS;
...
function statusBadgeTexture(indicator,reduced,overflow=0){
  ...
  if(statusBadgeTextures.has(cacheKey))return statusBadgeTextures.get(cacheKey);
  const size=128,canvas=document.createElement('canvas'); canvas.width=canvas.height=size;
  const ctx=canvas.getContext('2d');
  const bg=overflow?'#1e2b42':indicator.backgroundColor, border=overflow?'#cfd8e8':indicator.borderColor, ink=overflow?'#ffffff':indicator.color;
  ctx.clearRect(0,0,size,size); ctx.save(); ctx.translate(size/2,size/2); ctx.rotate(overflow?0:Math.PI/4); ctx.beginPath();
  if(indicator.cue==='cracked_diamond')ctx.rect(-40,-40,80,80); else ctx.arc(0,0,47,0,Math.PI*2);
  ctx.fillStyle=bg; ctx.globalAlpha=reduced ? .94 : .98; ctx.fill(); ctx.lineWidth=6; ctx.strokeStyle=border; ctx.stroke(); ctx.restore();
...
function clearStatusBadges(u){
  ...
  while(group.children.length){ const badge=group.children[0]; group.remove(badge); if(badge.material)badge.material.dispose(); }
...
function syncStatusIndicators(u,time){
  ...
  const active=u===G.active&&!G.over,turnPulse=active&&Number.isFinite(u._statusPulseStartedAt)?Math.max(0,1-(time-u._statusPulseStartedAt)/.9):0;
  const emphasis=getStatusIndicatorEmphasis({active,turnPulse,reducedGraphics:REDUCED_GRAPHICS,unitOpacity:u.mat.opacity});
  const baseScale=.47*(u.size>1?1.14:1)*frame.spriteScale;
  const scale=baseScale*(1+Math.sin(time*speed+(group.userData.phaseOffset||0)*.01)*amplitude)*emphasis.scale;
  badge.material.opacity=Math.max(transitionFade,emphasis.transitionOpacityFloor)*emphasis.opacityMultiplier; badge.position.set(0,statusBadgeAnchorY(u)+lift,.04); badge.scale.set(scale,scale,1);
}
const bossIntentBadgeTextures=new Map();
const bossIntentColors=Object.freeze({ultimateAccent:new THREE.Color(0xff4d36),ultimateDark:new THREE.Color(0x8c160f),chargeAccent:new THREE.Color(0xf2b84b),chargeDark:new THREE.Color(0x9a5b16)});
function bossIntentBadgeTexture(label,boss){
  const key=(boss?'boss:':'elite:')+label; if(bossIntentBadgeTextures.has(key))return bossIntentBadgeTextures.get(key);
  const canvas=document.createElement('canvas'); canvas.width=256; canvas.height=96; const ctx=canvas.getContext('2d'),ultimate=label==='ULTIME',accent=ultimate?'#ff6b4a':'#f2c86b';
  ctx.clearRect(0,0,256,96); ctx.fillStyle=boss?'rgba(28,8,8,.9)':'rgba(24,12,18,.86)'; ctx.strokeStyle=accent; ctx.lineWidth=boss?6:4; ctx.beginPath(); ctx.roundRect(9,12,238,72,18); ctx.fill(); ctx.stroke();
  ctx.fillStyle=accent; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.font=(boss?'700 34px ':'700 30px ')+'Cinzel, Georgia, serif'; ctx.fillText(label,128,50);
  const texture=new THREE.CanvasTexture(canvas); texture.colorSpace=THREE.SRGBColorSpace; texture.needsUpdate=true; bossIntentBadgeTextures.set(key,texture); return texture;
}
function ensureBossIntentBadge(u){
  if(u.bossIntentBadge)return u.bossIntentBadge;
  const material=new THREE.SpriteMaterial({transparent:true,depthWrite:false,depthTest:false,toneMapped:false,fog:false});
  const badge=new THREE.Sprite(material); badge.name='boss-intent-alert'; badge.renderOrder=COMBAT_RENDER_LAYERS.BOSS_ALERT; badge.visible=false; u.grp.add(badge); u.bossIntentBadge=badge; return badge;
}
function clearBossIntentPresentation(u,dispose=false){
  if(!u)return; u.focusOpacityFloor=undefined; u._bossIntentPreview=undefined; const badge=u.bossIntentBadge; if(badge)badge.visible=false;
  if(u._bossIntentVisualActive){ if(u.teamRing){u.teamRing.material.color.setHex(u.team==='player'?0x69d7ff:0xff5f52);u.teamRing.material.opacity=u.alive?COMBAT_PRESENTATION.units.teamRingOpacity:0;} if(u.teamGlow){u.teamGlow.material.color.setHex(u.team==='player'?0x69d7ff:0xff5f52);u.teamGlow.material.opacity=u.alive?(u.team==='player'?.16:.18):0;} if(u.outline&&u.outline.material){u.outline.material.color.setHex(0x03050a);u.outline.material.opacity=u.alive?Math.max(.16,u.mat.opacity*.44):u.outline.material.opacity;} }
  u._bossIntentVisualActive=false;
  if(dispose&&badge){ badge.removeFromParent(); if(badge.material)badge.material.dispose(); u.bossIntentBadge=null; }
}
function syncBossIntentPresentation(u,time){
  const state=resolveBossIntentVisualState({alive:u.alive,boss:u.boss,elite:u.elite,ap:u.ap,cooldown:u._ultCooldown,previewLevel:u._bossIntentPreview},REDUCED_GRAPHICS);
  if(!state){ clearBossIntentPresentation(u); return; }
  const badge=ensureBossIntentBadge(u),pulse=0.5+0.5*Math.sin(time*(state.level==='ultimate'?7.2:4.6)),boss=Boolean(u.boss||(!u.elite&&u._bossIntentPreview));
  u.focusOpacityFloor=state.minOpacity; u.mat.opacity=Math.max(u.mat.opacity,state.minOpacity); const texture=bossIntentBadgeTexture(state.label,boss); if(badge.material.map!==texture){badge.material.map=texture;badge.material.needsUpdate=true;} badge.material.opacity=.86+.14*pulse; badge.visible=true;
  badge.position.set(0,statusBadgeAnchorY(u)+(boss?.62:.52),.08); badge.scale.set(state.badgeScale*2.15,state.badgeScale*.72,1);
  const accent=state.level==='ultimate'?bossIntentColors.ultimateAccent:bossIntentColors.chargeAccent,dark=state.level==='ultimate'?bossIntentColors.ultimateDark:bossIntentColors.chargeDark;
  if(u.teamRing){ u.teamRing.material.color.copy(accent).lerp(dark,pulse*.32); u.teamRing.material.opacity=Math.max(u.teamRing.material.opacity,.82+state.ringPulse*pulse); }
  if(u.teamGlow){ u.teamGlow.material.color.copy(accent); u.teamGlow.material.opacity=Math.max(u.teamGlow.material.opacity,.18+state.ringPulse*pulse); }
  if(!REDUCED_GRAPHICS&&u.outline&&u.outline.material){ u.outline.material.color.copy(dark).lerp(accent,.22+.28*pulse); u.outline.material.opacity=Math.max(u.outline.material.opacity,.44+state.silhouettePulse*pulse); }
  u._bossIntentVisualActive=true;
}
function disposeBossIntentPresentation(u){ clearBossIntentPresentation(u,true); }
...
function buildSelectors(){
  ...
  selBase.rotation.x=-Math.PI/2; selBase.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SHADOW; selBase.visible=false; scene.add(selBase);
  ...
  selRingUnder.rotation.x=-Math.PI/2; selRingUnder.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SELECTOR; selRingUnder.visible=false; scene.add(selRingUnder);
  ...
  selRing.rotation.x=-Math.PI/2; selRing.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SELECTOR+1; selRing.visible=false; scene.add(selRing);
  ...
  hoverRingUnder.rotation.x=-Math.PI/2; hoverRingUnder.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SELECTOR; hoverRingUnder.visible=false; scene.add(hoverRingUnder);
  ...
  hoverRing.rotation.x=-Math.PI/2; hoverRing.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SELECTOR+1; hoverRing.visible=false; scene.add(hoverRing);
  ...
  faceArrow.rotation.x=Math.PI/2; faceArrow.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SELECTOR+2; faceArrow.visible=false; scene.add(faceArrow);
}
...
function createUnit(def){
  ...
  blob.rotation.x=-Math.PI/2; blob.position.y=0.04; blob.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SHADOW; grp.add(blob);
  ...
  teamGlow.rotation.x=-Math.PI/2; teamGlow.position.y=0.046; teamGlow.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SHADOW+1; grp.add(teamGlow);
  ...
  teamRingUnder.rotation.x=-Math.PI/2; teamRingUnder.position.y=0.052; teamRingUnder.renderOrder=COMBAT_RENDER_LAYERS.UNIT_SHADOW+2; grp.add(teamRingUnder);
...
function campaignDef(payload,index){
  ...
  return {
    ...
    weapons:(payload.weapons||[]).map(weapon=>({
      name:weapon.name||'Arme',icon:icons[weapon.type]||'⚔',weaponType:weapon.type,type:'phys',
...
function spawnUnits(){
  if(CHARACTER_SYSTEM_V2_QA){
    const hero=DEFS.find(def=>def.portrait==='/assets/characters/pixel/masters/alistair.png');
    const goblin=VISUAL_UNIT_TEMPLATES.goblin;
    const boss=BOSS_DEFS.find(def=>def.portrait==='/assets/characters/pixel/masters/lion_champion.png');
...
async function beginTurn(u){ if(G.over)return; G.active=u; G.pinnedUnit=null; hideActionPreview(); G.movedThisTurn=false; G.actedThisTurn=false; G.movedBeforeAct=false; G.skillMovedThisTurn=false; G.basicAttacksThisTurn=0; G.itemsUsedThisTurn=0; G.startGX=u.gx; G.startGZ=u.gz; u._usedUtility=false; u._statusPulseStartedAt=_t;
  ...
  if(sk){ if((u.boss||u.elite)&&u._ultCooldown<5){u._ultCooldown=5;clearBossIntentPresentation(u);} floatText(u,sk.name.toUpperCase()+' !',sk.col,true); logMsg(u.name+' est '+sk.name.toLowerCase()+' — tour passé.'); await wait(0.7); for(const s in u.statuses){ if(STATUS[s]&&STATUS[s].skip){ u.statuses[s]--; if(u.statuses[s]<=0)delete u.statuses[s]; } } refreshPanel(u); if(G.over)return; nextTurn(); return; }
...
function endTurn(){ if(G.busy||G.over)return; const u=G.active;
  ...
  restoreUnitFocus(); hideActionPreview(); closeMenus(); clearHL(); G._targetPreviewKey=null; G.pending=null; G.mode='idle'; nextTurn(); }
...
function getActionVfxPreset(spec={},u=null){
  ...
  if(spec.item){
    ...
    if(spec.cure)return 'support_holy_aura';
    if(spec.apRestore)return 'bless_aura';
    if(spec.flatDmg||vfxTextIncludes(text,['bombe','bomb','grenade']))return 'impact_explosion_large';
    if(spec.heal||spec.flatHeal)return 'heal_burst';
  }
  ...
  if(spec.revive)return 'support_revive_pillar';
  if(spec.heal)return 'heal_burst';
  if(spec.cure)return 'support_holy_aura';
  if(spec.key==='bulwark'||spec.key==='boss_guard'||spec.status==='barrier')return 'guard_barrier';
  if(spec.key==='bless'||spec.key==='regen'||spec.apRestore)return 'bless_aura';
...
function playActionVfxAt(presetId,u,targetPoint,spec={}){
  ...
  const point=targetPoint.clone?targetPoint.clone():new THREE.Vector3(targetPoint.x,targetPoint.y,targetPoint.z);
  const result=combatVfxSystem.play(presetId,{...makeActionVfxContext(u,[],u.gx,u.gz,spec),targetPoint:point});
  if(!result.played)return null;
  void result.completion.catch(error=>console.warn('[CombatVfx] Action playback failed safely.',error));
  return result;
}
...
async function doMove(u,spec,cx,cz){ setFacing(u,cx,cz); const c=cellAt(cx,cz); const dest=new THREE.Vector3(wX(cx),c.topY,wZ(cz)); const head=dest.clone().add(new THREE.Vector3(0,0.9,0));
  ...
  if(spec.mode==='teleport'){
    ...
    const leapHasSpritesheet=actionHasSpritesheetVfx(spec,u);
    if(leapHasSpritesheet)playActionVfxAt(getActionVfxPreset(spec,u),u,head,spec);
...
async function swapUnits(a,b,spec={}){
...
async function performSkillMovement(u,spec,cx,cz,context={}){
  ...
  if(spec.mode==='swap'){
    if(!await swapUnits(u,selected,spec)){ toast('Échange impossible'); return false; }
    return true;
  }
  if(spec.mode==='strike'){
...
async function executeActionCore(u,spec,cx,cz){ restoreUnitFocus(); hideActionPreview(); G.busy=true; closeMenus(); clearHL();
...
async function executeAction(u,spec,cx,cz){
  ...
  try { await executeActionCore(u,spec,cx,cz); }
    ...
    try { if(G.stage)await combatStageExit(); else restoreUnitFocus(); }
...
async function aiTurn(u){
  ...
  while(!G.over&&u.alive&&safety++<4){
    ...
    if(spec.ap>=5&&(u.boss||u.elite)){u._ultCooldown=5;clearBossIntentPresentation(u);}
...
function startNextWave(){ restoreUnitFocus(); for(const u of G.units)clearBossIntentPresentation(u); G.wave++;
...
function syncOptionCFrameAnimation(u,now){
  if(!u.runtimeFrameAnimation||!u.frameAnimation)return;
  const sample=u.frameAnimation.sample(now);
  const loaded=externalSpriteCache.get(sample.frameUrl);
  if(!loaded){ document.body.dataset.optionCAssetFailure=sample.frameUrl; return; }
  if(u.frameAnimationUrl!==sample.frameUrl){
    u.frameAnimationUrl=sample.frameUrl;
    u.mat.map=loaded.tex; u.mat.needsUpdate=true;
    if(u.outline){u.outline.material.map=loaded.tex;u.outline.material.needsUpdate=true;}
  }
  if(G.stage)combatStage.setCombatUnitTexture(u,loaded.tex);
  ...
  document.body.dataset.optionCCombatStageActive=String(combatStage.isActive());
}
function animate(){ if(_runtimeDisposed)return; _animationFrame=requestAnimationFrame(animate);
  ...
  for(const u of G.units){ syncOptionCFrameAnimation(u,now); const dx=camera.position.x-u.grp.position.x, dz=camera.position.z-u.grp.position.z; u.spr.rotation.y=Math.atan2(dx,dz); if(u.outline){ u.outline.rotation.y=u.spr.rotation.y; u.outline.material.opacity=Math.max(0.16,u.mat.opacity*0.44); }
  ...
  if(G.gridLines){
    ...
    const gridOp=GRID_DEBUG_ENABLED?1:(G.stage ? COMBAT_PRESENTATION.arena.gridOpacityStage : (tactical ? COMBAT_PRESENTATION.arena.gridOpacityTactical : (selected ? COMBAT_PRESENTATION.arena.gridOpacitySelected : COMBAT_PRESENTATION.arena.gridOpacityIdle)));
...
async function playOptionCStageProof(replayToken){
  ...
  placeUnit(owner,gx,target.gz,true);
  const spec=getSpec(owner,'attack',0,0);
  // Keep this isolated DEV encounter inspectable across repeated stage passes.
  target.hp=target.maxhp;
  owner.ap=Math.max(owner.ap,spec.ap);
  setOptionCAnimation('attack');
  document.body.dataset.optionCProofSurface='combat-stage';
  await executeAction(owner,spec,target.gx,target.gz);
  if(OPTION_C_PROOF_SURFACE==='combat-stage'&&replayToken===optionCStageReplayToken){
...
async function playQaMotionScenario(scenario){
  const bossAlert=scenario==='boss-charge'||scenario==='boss-ultimate'; qaPrepareCombat(); const source=qaMotionUnit(scenario==='boss'||bossAlert),target=aliveUnits(source&&source.team==='player'?'foe':'player')[0]||source; if(!source)return;
  G.busy=true;
  try{
    if(bossAlert){ clearQaBossIntentPreview(); source._bossIntentPreview=scenario==='boss-ultimate'?'ultimate':'charge'; syncBossIntentPresentation(source,_t); }
    else if(scenario==='hit')await playUnitHitReaction(source,{source:target});
