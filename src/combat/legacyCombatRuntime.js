...
function makeTileTex(){ hlTextures={
  range:buildTileTex('rgba(255,246,218,0.22)','rgba(186,177,128,0.10)','rgba(255,239,174,0.9)','rgba(97,92,61,0.88)','rgba(255,235,164,0.9)','rgba(226,211,146,0.6)',2.2,18),
  ...
  aoe:buildTileTex('rgba(255,230,180,0.28)','rgba(200,140,90,0.14)','rgba(255,220,160,0.85)','rgba(100,70,40,0.82)','rgba(255,215,150,0.85)','rgba(240,190,120,0.5)',2.4,18),
...
function addHL(gx,gz,color,op=0.45,kind='range'){ const c=cellAt(gx,gz); if(!c)return; const boost=kind==='target'||kind==='target_ally'?1.28:(kind==='hover'?1.24:(kind==='move'?1.22:(kind==='aoe'?1.10:(kind==='invalid'?1.08:1.14)))),finalOp=Math.min(1,op*boost);
  ...
  m.rotation.x=-Math.PI/2; m.position.set(wX(gx),c.topY+yOff,wZ(gz)); m.renderOrder=ro; m.userData.baseOp=finalOp; m.userData.pulse=kind==='target'||kind==='target_ally'||kind==='hover'?0.018:(kind==='aoe'?0.010:(kind==='invalid'?0.006:0.012)); hlGroup.add(m); hlMeshes.push(m); return m; }
...
async function beginTurn(u){ if(G.over)return; G.active=u; G.pinnedUnit=null; hideActionPreview(); G.movedThisTurn=false; G.actedThisTurn=false; G.movedBeforeAct=false; G.basicAttacksThisTurn=0; G.itemsUsedThisTurn=0; G.startGX=u.gx; G.startGZ=u.gz;
...
async function executeAction(u,spec,cx,cz){ unitFocus.restore(); hideActionPreview(); G.busy=true; closeMenus(); clearHL();
  ...
  if(isStandaloneMove){ await doMove(u,spec,cx,cz); applyAdditionalStatus(u,spec,cx,cz,context); if(spec.ap>0)u.ap=Math.max(0,u.ap-spec.ap); G.movedThisTurn=true; refreshPanel(u); restoreCam(); G.busy=false; checkEnd(); return; }
  ...
  if(moveBefore){ const moved=await performSkillMovement(u,spec,cx,cz,context); if(!moved){ restoreCam(); G.busy=false; return; } G.movedThisTurn=true; if(spec.mode!=='strike'&&spec.mode!=='swap'){ impactCx=u.gx; impactCz=u.gz; } }
  ...
  if(spec.effects&&spec.effects.length){
    ...
    for(const eff of spec.effects){
      if(eff.kind==='move'){ await doMove(u,{...spec,mode:eff.moveMode||spec.mode,type:'move',ap:0},impactCx,impactCz); G.movedThisTurn=true; continue; }
  ...
  if(hasSkillMovement&&!moveBefore){ const moved=await performSkillMovement(u,spec,cx,cz,context); if(moved)G.movedThisTurn=true; }
...
function drawRange(){ hideActionPreview(); clearHL(); const sp=G.pending.spec,keys=new Set(G.pending.centers.map(c=>cellKey(c.gx,c.gz))); addInvalidTiles(keys,false); for(const c of G.pending.centers) addHL(c.gx,c.gz,CFG.COL.path,COMBAT_PRESENTATION.arena.rangeTileOpacity,'range');
  const helpful=Boolean(sp.heal||sp.support||sp.revive||sp.cure);
...
function previewAt(cx,cz){ drawRange(); const sp=G.pending.spec,hoverCell=cellAt(cx,cz),hoverOcc=hoverCell&&hoverCell.occupant,helpful=Boolean(sp.heal||sp.support||sp.revive||sp.cure),hoverEnemy=hoverOcc&&G.active&&hoverOcc.team!==G.active.team,hoverAlly=hoverOcc&&G.active&&hoverOcc.team===G.active.team&&helpful; addHL(cx,cz,hoverEnemy?CFG.COL.foe:(hoverAlly?0x7edf7a:0xf7edcf),COMBAT_PRESENTATION.arena.hoverTileOpacity,hoverEnemy?'target':(hoverAlly?'target_ally':'hover')); if(hoverEnemy)addRingHL(cx,cz,CFG.COL.foe,COMBAT_PRESENTATION.arena.targetTileOpacity+.22); if(hoverAlly)addRingHL(cx,cz,0x7edf7a,COMBAT_PRESENTATION.arena.targetTileOpacity+.22);
  ...
  for(const [gx,gz] of aoeCells(G.active,sp,cx,cz)){ const occ=cellAt(gx,gz)?.occupant; let col=sp.type==='move'?CFG.COL.move:CFG.COL.path,op=COMBAT_PRESENTATION.arena.rangeTileOpacity,kind='aoe';
